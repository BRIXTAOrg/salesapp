...
class NativeTrackingRepository
    implements TrackingRepository {
  static const _channel =
      MethodChannel('kamdhenu/native_tracking');
  ...
  Future<TrackingSnapshot> status() async {
    ...
    final lastFixMs =
        (raw['lastFixAt'] as num?)?.toInt() ?? 0;

    return TrackingSnapshot(
      active: raw['active'] == true,
      distanceM:
          (raw['distanceM'] as num?)?.toDouble() ??
          0,
      employeeId: raw['employeeId']?.toString(),
      mode: raw['mode']?.toString() ?? 'idle',
      speedMps:
          (raw['speedMps'] as num?)?.toDouble() ??
          0,
